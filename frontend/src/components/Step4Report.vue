...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // 提取统计数据 - 匹配"相关预测Facts: X条"格式
    const factMatch = text.match(/相关预测Facts:\s*(\d+)/)
    ...
    // 提取Sub-questions - 完整提取，不限制数量
    const subQSection = text.match(/### 分析的Sub-questions\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
      const lines = subQSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.subQueries = lines.map(l => l.replace(/^\d+\.\s*/, '').trim()).filter(Boolean)
    }
    
    // 提取关键Facts - 完整提取，不限制数量
    const factsSection = text.match(/### 【关键Facts】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (factsSection) {
    ...
    // 提取Core entities - 完整提取，包含摘要和相关Facts数
    ...
    if (entitySection) {
      ...
      result.entities = entityBlocks.map(block => {
        ...
        const relatedMatch = block.match(/相关Facts:\s*(\d+)/)
        return {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    const activeMatch = text.match(/当前有效Facts:\s*(\d+)/)
    const histMatch = text.match(/历史\/过期Facts:\s*(\d+)/)
    ...
    // 提取当前有效Facts - 完整提取，不限制数量
    const activeSection = text.match(/### 【当前有效Facts】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
    ...
    // 提取历史/过期Facts - 完整提取，不限制数量
    const histSection = text.match(/### 【历史\/过期Facts】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (histSection) {
    ...
    // 提取Related entities - 完整提取，不限制数量
    const entitySection = text.match(/### 【Related entities】\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // 提取采访对象Selection rationale
    const reasonMatch = text.match(/### 采访对象Selection rationale\n([\s\S]*?)(?=\n---\n|\n### 采访实录)/)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }
    
    // 解析每个人的Selection rationale
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // 提取相关Facts - 完整提取，不限制数量
    const factsSection = text.match(/### 相关Facts:\n([\s\S]*)$/)
    if (factsSection) {
      const lines = factsSection[1].split('\n').filter(l => l.match(/^\d+\./))
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, '漂移查询生成分析Sub-questions'),
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, '暂无Key memories'),
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, '暂无Core entities'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, '暂无Relation chains')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, '暂无Active facts'),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, '暂无Historical facts'),
        ...
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, '暂无Related entities'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(No reply on this platform)' || t === '(该平台未获得回复)' || t === '[无回复]'
    }
    ...
    return () => h('div', { class: 'interview-display' }, [
      ...
      props.result.interviews.length > 0 && h('div', { class: 'interview-detail' }, [
        ...
        // Selection Reason - Selection rationale
    ...
    return () => h('div', { class: 'quick-search-display' }, [
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        activeTab.value === 'edges' && hasEdges.value && h('div', { class: 'edges-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, '相关Relations'),
