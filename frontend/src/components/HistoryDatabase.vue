  ...
  >
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        ...
        <!-- 卡片标题（使用Simulation requirement的前20字作为标题） -->
        <h3 class="card-title">{{ getSimulationTitle(project.simulation_requirement) }}</h3>

        <!-- 卡片描述（Simulation requirement完整展示） -->
        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <!-- 文件列表 -->
              <div class="modal-section">
                <div class="modal-label">Linked files</div>
                ...
                <div class="modal-empty" v-else>暂无Linked files</div>
              </div>
            </div>
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step 3 and Step 5 must be started live — not in replay</span>
            </div>
          </div>
        </div>
      </Transition>
...
const getSimulationTitle = (requirement) => {
  if (!requirement) return 'Unnamed模拟'
...
/* 不同功能的颜色 */
.status-icon:nth-child(1).available { color: #3B82F6; } /* Graph build - 蓝色 */
