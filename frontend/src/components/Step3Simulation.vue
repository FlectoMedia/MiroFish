...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // 从Step2传入的max rounds
  minutesPerRound: {
    type: Number,
    default: 30 // 默认per round30分钟
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const phase = ref(0) // 0: 未开始, 1: 运行中, 2: Done
...
// 格式化模拟流逝时间（根据round和per round分钟数计算）
...
const doStartSimulation = async () => {
  ...
  try {
    ...
    addLog('Dynamic graph updates enabled')
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ 已清理旧的模拟日志，重新Run simulation')
      }
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 检测模拟是否Done（通过 runner_status 或平台完成状态判断）
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    console.warn('Failed to get run detail:', err)
  }
}
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Report generation请求已发送，请稍候...')
    return
  }
  
  isGeneratingReport.value = true
  addLog('正在启动Report generation...')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ Report generation任务已启动: ${reportId}`)
      
      // 跳转到报告页面
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(`✗ 启动Report generation失败: ${res.error || 'Unknown error'}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`✗ 启动Report generation异常: ${err.message}`)
    isGeneratingReport.value = false
  }
}
