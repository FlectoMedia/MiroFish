...
const minutesPerRound = ref(30) // 默认per round30分钟
...
const handleGoBack = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        addLog(`Graceful close failed — force stopping...`)
        try {
  ...
  // 返回到 Step 2 (Environment setup)
  router.push({ name: 'Simulation', params: { simulationId: currentSimulationId.value } })
}

const handleNextStep = () => {
  // Step3Simulation 组件会直接处理Report generation和路由跳转
  // 这个方法仅作为备用
  addLog('进入 Step 4: Report generation')
}
    ...
    if (simRes.success && simRes.data) {
      ...
      try {
        ...
        if (configRes.success && configRes.data?.time_config?.minutes_per_round) {
          ...
          addLog(`Timing: ${minutesPerRound.value} min`)
        }
      } catch (configErr) {
        addLog(`Failed to load timing — default: ${minutesPerRound.value}min/round`)
      }
