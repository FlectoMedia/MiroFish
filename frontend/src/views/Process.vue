      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">Graph build starts when ontology is ready</p>
          </div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After upload, the LLM builds an ontology for opinion simulation (entity + relation types)
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">What it does</div>
                <div class="detail-content">
                  Chunks docs via Zep API to build the graph and extract entities and relations
                </div>
              </div>
...
// Current project id (may change from 'new')
...
const currentPhase = ref(-1) // -1: uploading, 0: ontology, 1: graph, 2: done
...
const statusText = computed(() => {
  ...
  if (currentPhase.value >= 2) return 'Complete'
  if (currentPhase.value === 1) return 'Building graph'
  if (currentPhase.value === 0) return 'Ontology in progress'
  return 'Initializing'
})
...
const goToNextStep = () => {
  // TODO: Continue to environment setup step
  alert('Environment setup is under development...')
}
...
// New project: ontology/generate
const handleNewProject = async () => {
  const pending = getPendingUpload()
  
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'No pending files — return home and try again'
    loading.value = false
    return
  }
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Keep polling in-flight build tasks
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Keep task_id for polling
      const taskId = response.data.task_id
      
      // Poll graph data (separate from task poll)
      startGraphPolling()
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        console.log('✅ Graph build done, loading full data...')
        
        stopPolling()
        stopGraphPolling()
        currentPhase.value = 2
        
        // Update progress UI to completed state
        buildProgress.value = {
          progress: 100,
          message: 'Build done — loading graph...'
        }
        
        // Reload project data for graph_id
        const projectResponse = await getProject(currentProjectId.value)
...
const renderGraph = () => {
  ...
  const nodes = nodesData.map(n => ({
    ...
    rawData: n // Keep raw row
  }))
  
  // Node id set for edges
