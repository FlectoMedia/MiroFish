...
接口1：分析文本内容，生成适合社会模拟的实体和Relations类型定义
"""
...
ONTOLOGY_SYSTEM_PROMPT = """Agent是一个专业的知识图谱本体设计专家。Agent的任务是分析给定的文本内容和Simulation requirement，设计适合**社交媒体舆论模拟**的Entity types和Relations类型。

**重要：Agent必须输出有效的JSON格式数据，不要输出任何Other内容。**
...
```json
{
    ...
    "edge_types": [
        {
            "name": "Relations类型名称（英文，UPPER_SNAKE_CASE）",
            "description": "简短描述（英文，不超过100字符）",
            "source_targets": [
...
### 1. Entity types设计 - 必须严格遵守

**数量要求：必须正好10个Entity types**

**层次结构要求（必须同时包含具体类型和兜底类型）**：

Agent的10个Entity types必须包含以下层次：

A. **兜底类型（必须包含，放在列表最后2个）**：
   - `Person`: 任何自然人个体的兜底类型。当一个人不属于Other更具体的人物类型时，归入此类。
   - `Organization`: 任何组织机构的兜底类型。当一个组织不属于Other更具体的组织类型时，归入此类。
...
- 数量：6-10个
- Relations应该反映社媒互动中的真实联系
- 确保Relations的 source_targets 涵盖Agent定义的Entity types

### 3. 属性设计

- 每个Entity types1-3个关键属性
...
class OntologyGenerator:
    ...
    def _build_user_message(
        ...
        # 如果文本超过5万字，截断（仅影响传给LLM的内容，不影响Graph build）
...
"""
        ...
        message += """
请根据以上内容，设计适合社会舆论模拟的Entity types和Relations类型。

**必须遵守的规则**：
1. 必须正好输出10个Entity types
...
"""
    ...
    def generate_python_code(self, ontology: Dict[str, Any]) -> str:
        ...
        code_lines.append('# ============== Relations类型定义 ==============')
        code_lines.append('')
